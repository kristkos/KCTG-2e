Hooks.on('init', () => {
    game.settings.register('kctg-2e','welcome', {scope:'world',type:Boolean,config:false,default:false})
})
Hooks.on('ready', () => {
    if (game.settings.get('kctg-2e','welcome') || !game.user.isGM) return // Skip sending message if this has already happened
    ...
    game.settings.set('kctg-2e','welcome',true)
})